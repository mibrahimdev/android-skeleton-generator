...
reset_config() {
  ...
  NETWORK_LIB="" MOCK_LIB="" MODULE_TYPE="" OUTPUT_DIR="" NON_INTERACTIVE=""
}

reset_config
parse_args --non-interactive --name TestApp --package com.test.myapp \
  --arch mvvm-clean --di hilt --net retrofit --mock mockk --min-sdk 24 --modules single --output /tmp/test
...
assert_eq "parse mock" "mockk" "$MOCK_LIB"
...
MOCK_LIB="mockk"; MODULE_TYPE="single"; OUTPUT_DIR="/tmp/test"
...
for mock in mockk mockito; do
  MOCK_LIB="$mock"
  assert_eq "accept mock $mock" "0" "$(validate_config 2>/dev/null && echo 0 || echo 1)"
done
MOCK_LIB="mockk"

# Bad mock
MOCK_LIB="powermock"
assert_eq "reject bad mock" "1" "$(validate_config 2>/dev/null && echo 0 || echo 1)"
MOCK_LIB="mockk"
